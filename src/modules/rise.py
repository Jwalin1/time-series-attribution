...
class RISE(nn.Module):
    def __init__(self, model, n_masks=1000, p1=0.1, input_size=224, initial_mask_size=7, n_batch=128, mask_path=None):
        super().__init__()
        self.model = model
    ...
    def generate_masks(self):
        ...
        Ch = np.ceil(self.input_size / self.initial_mask_size)

        resize_h = int((self.initial_mask_size + 1) * Ch)
        ...
        for _ in range(self.n_masks):
            ...
            binary_mask = torch.randn(
                1, 1, self.initial_mask_size)
            binary_mask = (binary_mask < self.p1).float()

            # upsampling mask
            mask = F.interpolate(
                binary_mask, (resize_h))

            # random cropping
            i = np.random.randint(0, Ch)
            mask = mask[:, :, i:i+self.input_size]
