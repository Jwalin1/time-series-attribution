...
class GradCAMpp(CAM):
    ...
    def getGradCAMpp(self, values, score, idx):
        ...
        denominator += ag.view(n, c, -1).sum(-1, keepdim=True).view(n, c, 1)
        denominator = torch.where(
            denominator != 0.0, denominator, torch.ones_like(denominator))
        alpha = numerator / (denominator + 1e-7)

        relu_grad = F.relu(score[0, idx].exp() * gradients)
        weights = (alpha * relu_grad).view(n, c, -1).sum(-1).view(n, c, 1)
...
class SmoothGradCAMpp(CAM):
    ...
    def forward(self, x, idx=None):
        ...
        for i in range(self.n_samples):
            ...
            ag = activations * gradients.pow(3)
            denominator += \
                ag.view(n, c, -1).sum(-1, keepdim=True).view(n, c, 1)
            denominator = torch.where(
                denominator != 0.0, denominator, torch.ones_like(denominator))
            alpha = numerator / (denominator + 1e-7)
            ...
            relu_grad = F.relu(score[0, idx].exp() * gradients)
            weights = \
                (alpha * relu_grad).view(n, c, -1).sum(-1).view(n, c, 1)
